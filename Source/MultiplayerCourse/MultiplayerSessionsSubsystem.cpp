...
void UMultiplayerSessionsSubsystem::OnFindSessionsComplete(bool bWasSuccesful)
{
	...
	if(Results.Num() > 0)
	{
		...
		for (FOnlineSessionSearchResult Result : Results)
		{
			if(Result.IsValid())
			{
				...
				if(ServerName.Equals(ServerNameToFind))
				{
					...
					FString Msg2 = FString::Printf(TEXT("Found server with a name: %s"), *ServerName);
					PrintString(Msg2);
					break;
				}
			}
		...
		{
			FString Msg2 = FString::Printf(TEXT("Couldn't find server with a name: %s"), *ServerNameToFind);
			ServerNameToFind = "";
		}
	}
	else
